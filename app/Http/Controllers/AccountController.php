...
class AccountController extends Controller
{
    ...
    public function closing(Request $request)
    {
        ...
        foreach ($accounts as $account) {
            foreach ($account->transactions as $transaction) {
                if ($transaction->journal_voucher->status == 'unposted') {
                    return redirect()->back()->with('danger', 'Please post all journal vouchers before closing!');
                }
            }
        }
        ...
        $jv = JournalVoucher::create([
            ...
            'rate' => $lbp->rate,
            'date' => $datetime,
            'description' => 'Closing Journal Voucher from ' . $request->from_date . ' to ' . $request->to_date . ' for accounts ' . $request->from_account . ' to ' . $request->to_account . ' Profit account ' . $request->profit_account . ' Loss account ' . $request->loss_account .  ', automactically generated by system...',
            'status' => 'posted',
            'source' => 'system',
            'batch' => 'S',
            'created_at' => $datetime,
