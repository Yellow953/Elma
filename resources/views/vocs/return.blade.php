...
<div class="inner-container w-100 m-0 p-5">
    <div class="card">
        <div class="card-header bg-info border-b">
            <h4 class="font-weight-bolder">Return VOC</h4>
        </div>
        <div class="card-body">
            <div class="container px-0 px-md-5">
                ...
                <form role="form" action="{{ route('voc.return_save') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row setup-content" id="step-1">
                        <div class="offset-md-2 col-md-8">
                            ...
                            <div class="row mt-4">
                                <div class="col-md-12">
                                    <p class="text-center">Please select the VOC you want to return</p>
                                </div>
                                <div class="form-group">
                                    <label class="control-label">VOC</label>
                                    <select name="voc_id" id="voc_id" required="required" class="form-select select2">
                                        <option value=""></option>
                                        @foreach ($vocs as $voc)
                                        <option value="{{ $voc->id }}" {{ $voc->id == old('voc_id') ?
                                            'selected':'' }}>{{ $voc->voc_number }}</option>
                                        @endforeach
                                    </select>
                                </div>
                    ...
                    <div class="row setup-content" id="step-2">
                        <div class="offset-md-2 col-md-8">
                            <div class="col-md-12">
                                ...
                                <div class="col-md-12">
                                    <p class="text-center">
                                        Please select the items you want to return form this VOC
                                    </p>
                                </div>
...
<script>
    $(document).ready(function () {
        ...
        $('#voc_id').on('change', function () {
            var voc_id = $(this).val();
            if (voc_id) {
                $.ajax({
                    url: '/voc/' + voc_id + '/items',
                    type: 'GET',
                    dataType: 'json',
